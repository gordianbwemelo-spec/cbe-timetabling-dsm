...
// year (4 = Year 2, otherwise Year 1). Returns '' when the code doesn't match.
function codeLevel(code){
  const m=/^[A-Za-z]+(\d{2})(\d)/.exec((code||'').trim());
  if(!m)return '';
  const blk=parseInt(m[1],10), sem=m[2];
...
function ntaSelectOpts(sel){
  return '<option value=""'+(sel?'':' selected')+'>any level</option>'+
    NTAOPTS.map(n=>`<option value="${n}"${n===sel?' selected':''}>${n}</option>`).join('');
}
// Full programme names shown next to each code on the Enrolment page.
// Unknown codes just show the code itself.
const PROGFULL={
  ...
  ICT:'Information and Communication Technology', MET:'Metrology',
  MK:'Marketing', PSCM:'Procurement and Supply Chain Management',
  RAM:'Records and Archives Management', TLM:'Transport and Logistics Management',
  MSCM:'Master of Supply Chain Management',
  'MSc.ITPMGT':'MSc in IT Project Management',
  'Msc. PSCM':'MSc in Procurement and Supply Chain Management',
  'MBA-F&B':'MBA – Finance and Banking', 'MBA-HRM':'MBA – Human Resource Management',
  'MBA-MKTM':'MBA – Marketing Management',
  BBSE:'Business Studies with Education',
  MIBM:'Master of International Business Management',
  MLG:'Master of Leadership and Governance',
...
// When a module is chosen, auto-fill the NTA level from its code (still editable).
function tmodPick(){const el=$('tmod');if(!el)return;const code=(window.__modByName&&window.__modByName[el.value.trim()])||'';const lv=codeLevel(code);const sel=$('tnta');if(sel&&lv)sel.value=lv;}
async function teachAdd(){const el=$('tmod');const mod=el.value.trim();if(!mod){el.focus();return;}
  ...
  toast('Added module for '+window.__tInstr+(nta?' ('+nta+')':' (any level)'));renderTeaching();}
async function teachSetNta(id,nta){const r=(TEACHROWS||[]).find(x=>x._id===id);if(!r)return;
  await api('/ref/teaching/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({instructor:r.instructor,code:r.code,module:r.module,nta,sem:SEM})});
  toast('NTA level updated'+(nta?' → '+nta:''));renderTeaching();}
// Edit a module's code against its name; the NTA level auto-updates from the new code.
async function teachSetCode(id,code){const r=(TEACHROWS||[]).find(x=>x._id===id);if(!r)return;
  code=code.trim();const lv=codeLevel(code);const nta=lv||r.nta||'';
  await api('/ref/teaching/'+id,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({instructor:r.instructor,code,module:r.module,nta,sem:SEM})});
